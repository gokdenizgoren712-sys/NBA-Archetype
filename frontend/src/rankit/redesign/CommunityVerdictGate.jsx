...
export default function CommunityVerdictGate({ spoiler = false, onReveal, actionLabel = "REVEAL ANYWAY", message }) {
  return (
    <div data-community-gate="" style={{ margin: "12px 0", padding: "14px 16px", border: "1px dashed rgba(255,255,255,.22)", borderRadius: 10, background: "rgba(9,10,11,.5)", display: "grid", gap: 11 }}>
      ...
      <span style={{ color: "#c9cccd", font: "400 12px/1.45 var(--font-sans)" }}>
        {message || (spoiler ? "CONTAINS SPOILERS · TAP TO SHOW" : "Rate it first — then see whether the room agreed with you.")}
      </span>
      <button type="button" className="ri-gate-action" onClick={onReveal} style={{ justifySelf: "start", border: 0, background: "none", color: "#9aa0a6", padding: 0, font: "700 11px var(--font-logo)", letterSpacing: ".09em", cursor: "pointer" }}>
        {actionLabel}
      </button>
    </div>
  );
