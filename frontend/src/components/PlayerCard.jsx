...
const CORE = ["Engine","Anchor","Rim Runner","Spacer","Connector","Creator"];

const ARCH_COLOR = {
  Engine:      "bg-orange-500/20 text-orange-300 border-orange-500/30",
  Anchor:      "bg-blue-500/20 text-blue-300 border-blue-500/30",
  "Rim Runner":"bg-green-500/20 text-green-300 border-green-500/30",
    ...
    >
        ...
        <div className="text-right">
          {hasOverall ? (
            <>
              ...
              {pct != null && (
                <div className="text-[10px] text-slate-500">top {100 - pct}%</div>
              )}
            </>
          ) : (
            <>
      ...
      <div className="space-y-1">
        {scoreEntries.map(([comp, val]) => (
          <ScoreBar key={comp} label={comp} value={val} highlight={CORE.includes(comp)} />
        ))}
      </div>
    </div>
  );
