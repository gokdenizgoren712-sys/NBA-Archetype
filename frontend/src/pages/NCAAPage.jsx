...
// Initiator burada YOK — NCAA'de optik takip verisi (hız/mesafe/tempo) hiç
// gelmiyor, formülünün %85'i sessizce eksik kalıyordu (2026-07 denetimi).
...
export default function NCAAPage() {
  ...
  const secondaryCount = [conference, pos, arch, tier, minGp, maxAge].filter(Boolean).length;
  ...
  return (
    ...
    <div className="relative flex flex-col h-full min-h-0 overflow-hidden">
      ...
      <div className="mx-4 mb-2 px-3 py-2 rounded-lg text-[11px] leading-relaxed shrink-0"
        ...
        NCAA has no optical tracking data (speed, distance, pace), so the <strong>Initiator</strong> archetype
      ...
      <div className={`aura-glass absolute top-0 bottom-0 left-0 z-50 w-72 max-w-[85vw] flex flex-col rounded-r-2xl transition-transform duration-300 ease-out
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-4">
          {filterField("Conference", selectEl(conference, setConference, confList, "Any conference", { full: true }))}
          ...
          {filterField("Max Age", (
            <input type="number" min="0" value={maxAge} onChange={e => setMaxAge(e.target.value)}
              placeholder="e.g. 22" className="aura-ghost-input w-full" />
          ))}
        </div>

        {secondaryCount > 0 && (
          <div className="px-4 pb-4 shrink-0">
            <button onClick={() => { setConference(""); setPos(""); setArch(""); setTier(""); setMinGp(""); setMaxAge(""); }}
